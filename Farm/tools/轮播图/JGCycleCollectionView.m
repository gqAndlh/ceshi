...
@interface JGCycleCollectionView () <UICollectionViewDelegate,UICollectionViewDataSource>
...
static NSString * const JGCycleCollectionViewCellId = @"JGCycleCollectionViewCellId";

@implementation JGCycleCollectionView
...
- (void)buildUI {
    ...
    [self.collectionView registerClass:[JGCycleCollectionViewCell class] forCellWithReuseIdentifier:JGCycleCollectionViewCellId];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    JGCycleCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:JGCycleCollectionViewCellId forIndexPath:indexPath];
    NSString *str=self.titles[indexPath.row];
    if ([[str substringToIndex:4]isEqualToString:@"http"]) {
        [cell.Icon sd_setImageWithURL:[NSURL URLWithString:[self.titles objectAtIndex:indexPath.row]] placeholderImage:[UIImage imageNamed:@"lbt"]];
    }else{
        cell.Icon.image = [UIImage imageNamed:[self.titles objectAtIndex:indexPath.row]];
    }
...
{
    self.URLBlock(self.titles[indexPath.row]);
}
